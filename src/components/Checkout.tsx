...
const StripePaymentForm: React.FC<{ onSuccess: () => void }> = ({ onSuccess }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6 mt-4">
            ...
            <button disabled={!stripe || isProcessing} type="submit" className={btnLiquidClass}><span className={btnLiquidHover}></span><span className={btnLiquidContent}>{isProcessing ? <Loader2 className="animate-spin" size={18} /> : <><CreditCard size={18} /> Paga Adesso</>}</span></button>
            <div className="flex justify-center gap-2 items-center text-[10px] text-slate-400 font-bold uppercase tracking-wider opacity-60"><ShieldCheck size={12} /> Transazione Sicura SSL 256-bit</div>
        </form>
    );
};

const Checkout: React.FC<CheckoutProps> = ({ onBack }) => {
  const { cart, cartTotal, addOrder, shippingConfig, stripeConfig, clearCart, mailConfig, orders, applyCoupon, removeCoupon, appliedCoupon, cartDiscountAmount } = useStore();
  ...
  useEffect(() => { if (stripeConfig.publicKey) setStripePromise(loadStripe(stripeConfig.publicKey)); }, [stripeConfig]);
  useEffect(() => { window.scrollTo({ top: 0, behavior: 'instant' }); }, []);

  const subTotal = cartTotal(); // Già include coupon
  const isItaly = countryCode === 'IT';
  const shippingCost = isItaly ? (subTotal >= shippingConfig.italyThreshold ? 0 : shippingConfig.italyPrice) : (subTotal >= shippingConfig.foreignThreshold ? 0 : shippingConfig.foreignPrice);
  const grandTotal = subTotal + shippingCost;
  const discountVal = cartDiscountAmount();

  const handleApplyCoupon = () => {
      if(!couponCode) return;
      const success = applyCoupon(couponCode);
      if(success) setCouponMsg('Coupon applicato!');
      else setCouponMsg('Coupon non valido.');
  };
  ...
  const handleAddressSubmit = async (e: React.FormEvent) => {
    ...
    setServerError(null);
    if (wantInvoice && !form.taxId && !form.vatNumber) { setServerError("Codice Fiscale o P.IVA obbligatori."); return; }
    if (!privacyAccepted || !termsAccepted) { alert("Accetta Privacy e Termini."); return; }
    setIsProcessing(true);
    const newOrderPayload = {
        customerEmail: form.email, customerName: `${form.firstName} ${form.lastName}`, shippingAddress: `${form.address}, ${form.city} ${form.zip} (${COUNTRIES.find(c => c.code === countryCode)?.name})`,
        ...
        total: grandTotal, shippingCost: shippingCost, status: 'pending'
    };
    try {
        const createdOrder = await addOrder(newOrderPayload as any);
        if (!createdOrder) throw new Error("Errore creazione ordine");
        setOrderId(createdOrder.id);
        const res = await fetch('/api/payment', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ orderId: createdOrder.id }) });
        const data = await res.json();
        if (data.error) throw new Error(data.error);
        setClientSecret(data.clientSecret);
        setStep(2);
        setIsProcessing(false);
    } catch (err: any) { setIsProcessing(false); setOrderId(null); setServerError(err.message || "Errore. Riprova."); }
  };

  const handleSuccess = async () => {
      if (orderId) { const currentOrder = orders.find(o => o.id === orderId); if (currentOrder) await sendOrderConfirmationEmail(mailConfig, currentOrder, form.email, `${form.firstName} ${form.lastName}`); }
  ...
  if (isSuccess) return (
      <div className="pt-40 pb-20 min-h-screen flex flex-col items-center justify-center text-center px-6 bg-white animate-in fade-in zoom-in duration-300">
        ...
        <button onClick={onBack} className={btnLiquidClass.replace('w-full', 'w-auto min-w-[240px]')}><span className={btnLiquidHover}></span><span className={btnLiquidContent}><ArrowLeft size={16} /> Torna alla Home</span></button>
      </div>
  );
