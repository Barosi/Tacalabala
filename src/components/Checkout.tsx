...
import { ArrowLeft, CheckCircle, Loader2, Globe, Lock, AlertCircle, Truck, FileText, CreditCard, ShieldCheck, Edit2 } from 'lucide-react';
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6 mt-4">
            ...
            <button 
                ...
                type="submit" 
                className={btnLiquidClass}
            >
                <span className={btnLiquidHover}></span>
                <span className={btnLiquidContent}>
...
const Checkout: React.FC<CheckoutProps> = ({ onBack }) => {
  ...
  const handleEditStep1 = () => {
      // Per semplicità UI: permette di tornare indietro per modificare.
      // Lato backend, l'ordine "pending" creato rimarrà appeso fino a che Admin non lo cancella o scade.
      // Se l'utente poi riclicca "Vai al pagamento", si creerà un nuovo ordine.
      // È un compromesso accettabile per non complicare eccessivamente la logica di update ordine esistente.
      setStep(1);
      setOrderId(null);
  ...
  if (isSuccess) {
    return (
      <div className="pt-40 pb-20 min-h-screen flex flex-col items-center justify-center text-center px-6 bg-white animate-in fade-in zoom-in duration-300">
        ...
        <button onClick={onBack} className={btnLiquidClass.replace('w-full', 'w-auto min-w-[240px]')}>
            <span className={btnLiquidHover}></span>
            <span className={btnLiquidContent}>
  ...
  const stripeAppearance = {
    ...
    variables: { colorPrimary: '#0066b2', colorBackground: '#ffffff', colorText: '#0f172a', fontFamily: 'Inter, sans-serif', borderRadius: '12px', colorDanger: '#ef4444' },
    rules: {
        '.Tab': { border: '1px solid #e2e8f0', boxShadow: 'none' },
        '.Tab:hover': { color: '#0f172a' },
        '.Tab--selected': { borderColor: '#0066b2', boxShadow: '0 0 0 1px #0066b2' },
        '.Input': { border: '1px solid #e2e8f0', boxShadow: '0 1px 2px 0 rgba(0, 0, 0, 0.05)' },
        '.Input:focus': { border: '1px solid #0066b2', boxShadow: '0 0 0 1px #0066b2' }
    }
  };
          ...
          <div className="lg:col-span-7 space-y-8">
             ...
             <div className={`bg-white p-8 md:p-10 rounded-[2.5rem] shadow-xl shadow-blue-900/5 border transition-all duration-500 ${step === 1 ? 'border-[#0066b2] ring-4 ring-blue-50' : 'border-slate-100 opacity-60'}`}>
                 <div className="flex items-center justify-between mb-8 pb-6 border-b border-slate-100">
                    ...
                    {step === 2 && (
                        <button onClick={handleEditStep1} className="flex items-center gap-2 text-xs font-bold uppercase text-slate-500 hover:text-[#0066b2] bg-slate-100 hover:bg-white px-3 py-1.5 rounded-lg border border-transparent hover:border-slate-200 transition-all">
                             ...
                             {wantInvoice && (
                                 <div className="bg-slate-50 p-6 rounded-3xl border border-slate-200 animate-in fade-in space-y-4">
                                     ...
                                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                         <div><label className={labelClass}>Codice Fiscale (Privati)</label><input type="text" className={inputClass} value={form.taxId} onChange={e => setForm({...form, taxId: e.target.value})} placeholder="RSSMRA80A01H501U" /></div>
                                         <div><label className={labelClass}>Partita IVA (Aziende)</label><input type="text" className={inputClass} value={form.vatNumber} onChange={e => setForm({...form, vatNumber: e.target.value})} placeholder="IT12345678901" /></div>
                                     </div>
                                     <div><label className={labelClass}>Codice SDI o PEC (Opzionale)</label><input type="text" className={inputClass} value={form.sdiCode} onChange={e => setForm({...form, sdiCode: e.target.value})} placeholder="0000000 o nome@pec.it" /></div>
                                 </div>
                             )}
                        </div>
                 ...
                 {step === 2 && clientSecret && stripePromise && (
                     <div className="animate-in fade-in slide-in-from-bottom-4 space-y-8">
                         ...
                         <div className="bg-[#fcfcfc] p-6 rounded-2xl border border-slate-100">
                            <p className="text-[10px] font-bold uppercase text-slate-400 mb-4 tracking-widest">PayPal</p>
                            <PayPalScriptProvider options={{ clientId: "test", currency: "EUR" }}>
                                <PayPalButtons 
                                    style={{ layout: "vertical", height: 48, shape: 'pill', color: 'gold', label: 'paypal' }} 
                         ...
                         <div className="relative flex py-2 items-center">
                             ...
                             <span className="flex-shrink-0 mx-4 text-slate-300 text-[10px] uppercase font-bold tracking-widest">Oppure Carta di Credito</span>
                             <div className="flex-grow border-t border-slate-200"></div>
                         </div>
